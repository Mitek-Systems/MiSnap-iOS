...
//  MiSnap
//
//  Created by Panda Rajaram on 10/8/21.
//
...
extension ViewController {
    ...
    @objc private func buttonClicked() {
        ...
        guard let parameters = MiSnapParameters(defaultParametersFor: .anyId) else { return }

//        if licenseKeyNotValid(for: parameters) { return }

        vc = MiSnapViewController(with: parameters, delegate: self)
    ...
    private func licenseKeyNotValid(for parameters: MiSnapParameters) -> Bool {
        ...
        if parameters.documentTypeCategory == .deposit && !MiSnapLicenseManager.shared().featureSupported(.depositCapture) || parameters.documentTypeCategory == .id && !MiSnapLicenseManager.shared().featureSupported(.idCapture) {
