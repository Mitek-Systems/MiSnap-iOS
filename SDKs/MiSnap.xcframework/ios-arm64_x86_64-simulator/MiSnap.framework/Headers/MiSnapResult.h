...
typedef NS_ENUM(NSInteger, MiSnapStatus) {
    ...
    /**
     ID Front is expected but some other document is found
     
     Requires `MiSnapLicenseFeatureODC` to be supported
     */
    MiSnapStatusNotIdentityFront            = 2,
    /**
     ID Back is expected but some other document is found
    ...
    /**
     One side of a document is much closer to a camera than others
     */
...
typedef NS_ENUM(NSInteger, MiSnapResultCode) {
    ...
    /**
     A session was completed by manual acquiring an image
     */
    MiSnapResultCodeSuccessStillCamera    = 1,
    /**
     A session was completed by automatically determining an image that passed Image Quality Analysis checks
     */
