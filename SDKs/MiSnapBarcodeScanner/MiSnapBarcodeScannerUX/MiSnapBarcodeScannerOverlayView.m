//
//  MiSnapBarcodeScannerOverlayView.m
...
@interface MiSnapBarcodeScannerOverlayView ()
...
@implementation MiSnapBarcodeScannerOverlayView
...
- (void)torchEnabled:(BOOL)torchIsOn
{
    if (torchIsOn)
    {
        ...
        [self.torchButton setImage:[UIImage imageNamed:@"barcode_icon_flash_on"] forState:UIControlStateNormal];
    }
    else
    {
        self.torchButton.accessibilityLabel = [self localizedStringForKey:@"misnap_barcode_flash_off"];
        [self.torchButton setImage:[UIImage imageNamed:@"barcode_icon_flash_off"] forState:UIControlStateNormal];
    }
}
...
- (void)updateWithOrientation:(UIInterfaceOrientation)orientation
{
    ...
    switch (self.orientation)
    {
        ...
        case UIInterfaceOrientationPortraitUpsideDown:
            if (self.guideMode == MiSnapBarcodeGuideModeDevicePortraitGuidePortrait)
            {
            ...
            else if (self.guideMode == MiSnapBarcodeGuideModeDevicePortraitGuideLandscape)
            {
...
- (void)setGuideMode:(MiSnapBarcodeGuideMode)guideMode
{
    if (guideMode < MiSnapBarcodeGuideModeDevicePortraitGuidePortrait || guideMode > MiSnapBarcodeGuideModeNoGuide)
    {
        _guideMode = MiSnapBarcodeGuideModeDevicePortraitGuidePortrait;
    }
    else
    {
...
{
    return [[NSBundle bundleForClass:[self class]] localizedStringForKey:key value:key table:@"MiSnapBarcodeScannerLocalizable"];
}
